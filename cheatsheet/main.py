...
from func import nan_replace, standardizare_centrala, tabelare_matrice, calcul_corelatii_covariante

pd.set_option("display.max_columns", None)
np.set_printoptions(5,threshold=sys.maxsize,suppress=True)

tabel_date=pd.read_csv("in/Teritorial_2022.csv",index_col=0)# indexul va fi coloana 0 in de generarea altui index
...
tabelare_matrice(r,variabile_numerice,variabile_numerice,"out/date.out")
v=np.cov(x,rowvar=False)
tabelare_matrice(v,variabile_numerice,variabile_numerice,"out/corelatii.csv")#1 pe diag principala
...
for v in r_v:
    ...
    print("Corelatii ")
    print(r_v[v][0])
    tabelare_matrice(r_v[v][0],variabile_numerice,variabile_numerice,"out/V"+v+".csv")
    print("covarianta " )
    print(r_v[v][1])
    tabelare_matrice(r_v[v][1], variabile_numerice, variabile_numerice, "out/R" + v + ".csv")